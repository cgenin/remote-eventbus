...
public class SocketImplShould {
    ...
    public void connectBetweenServerAndClient() throws Exception {
        ...
        final RemoteEventBusServer server = new RemoteEventBusServer.Builder().groupName("TEST").useSocket().launch();
        final Listen listen = new Listen();
        server.register(listen);


        final RemoteEventBusClient client = new RemoteEventBusClient.Builder().port(server.getPort()).groupName(server.getGroupName()).useSocket().build();
    ...
    public void notBeSendIfThePortMismatch() throws Exception {
        ...
        final RemoteEventBusClient client = new RemoteEventBusClient.Builder()
                .port(server.getPort() + 1).groupName(server.getGroupName()).useSocket().build();
        final MyBean msg = new MyBean();
        msg.setName("Result OK");
    ...
    public void notBeSendIfTheGroupMismatch() throws Exception {
        ...
        final RemoteEventBusClient client = new RemoteEventBusClient.Builder()
                .port(server.getPort()).groupName(server.getGroupName()+"0").useSocket().build();
        final MyBean msg = new MyBean();
        msg.setName("Result OK");
